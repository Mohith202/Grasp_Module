...
TEST_DATASET = GraspNetDataset(cfgs.dataset_root, valid_obj_idxs, grasp_labels, camera=cfgs.camera, split='test', num_points=cfgs.num_point, remove_outlier=True, augment=False)
...
#         # Compute loss
#         loss, end_points = get_loss(end_points)
#         print(end_points['objectness_score'][0],"objectness_score")
#         print(batch_data_label['objectness_label'][0],"objectness_label")
#         # Collect true labels and predicted scores for AP calculation
#         all_true_labels.append(batch_data_label['objectness_label'].cpu().numpy())
#         all_pred_scores.append(end_points['objectness_score'].cpu().numpy())
...
#     y_scores = np.concatenate(all_pred_scores)
#     ap = calculate_ap(y_true, y_scores)
#     log_string('eval mean AP: %f' % ap)
